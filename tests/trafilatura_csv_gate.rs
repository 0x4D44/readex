...
use readex::{extract_to_csv, Options};
...
fn trafilatura_csv_gate() {
    ...
    for fixture_rel in FIXTURES {
        ...
        let python_csv_raw = match run_oracle("--csv", &path) {
            Ok(s) => s,
            Err(e) => panic!(
        ...
        let rust_nfc: String = nfc(&rust_csv_raw);
        let python_nfc: String = nfc(&python_csv_raw);
