//! M7 Stage 2 — corpus-wide JSON equivalence diff harness.
//!
//! Sibling of `trafilatura_txt_gate` / `trafilatura_markdown_gate`: this gate
//! pins mdrcel's `extract_to_json` against Python's
//! `trafilatura.extract(raw, output_format="json")` byte-for-byte.
//!
//! JSON is the **structured** sibling of txt: Python's `core.py:66-67` routes
//! the extracted `Document` through `build_json_output(document,
//! options.with_metadata)` (`xml.py:115-134`). With `with_metadata=False`
//! (the default the oracle and `Options::default()` both use), the output is
//! the body-only object `{"text": <xmltotxt(body)>, "comments":
//! <xmltotxt(commentsbody)>}` serialised via `json.dumps(..,
//! ensure_ascii=False)` — DEFAULT separators `", "` / `": "`. The `text`
//! payload is the SAME `xmltotxt(body, include_formatting=False)` string the
//! txt gate compares, just JSON-wrapped + escaped, so divergences should track
//! the txt gate closely; if many NEW ones appear, suspect a harness/serialiser
//! bug before a real divergence.
//!
//! # Comparison shape
//!
//! Both sides emit a Python/Rust `str` (a JSON document). The Python side
//! NFC-normalises its text payload upstream (`core.py:98` runs on the txt
//! body before json wrapping for the non-metadata branch — and the body text
//! itself is `xmltotxt` which mdrcel also NFC-normalises in `xmltotxt`). The
//! harness NFC-normalises both whole JSON strings ONCE MORE (belt-and-braces)
...
use mdrcel::{extract_to_json, Options};
use unicode_normalization::UnicodeNormalization;

/// Fixtures where Python's `trafilatura.extract(output_format="json")` is the
...
/// `wrk_docs/m7-allowlist/`. The JSON `text` payload is the same
/// `xmltotxt(body)` string the txt gate diffs, so these five share their root
/// cause with the txt + markdown gates — the divergence is format-independent
/// (selection/parser/decoding, not formatting). Each cross-references the
...
const PYTHON_UNDER_EXTRACT_ALLOWLIST: &[&str] = &[
    ...
    // structurally-valid filing (upstream of the json branch); mdrcel
...
/// Fixtures where **mdrcel** is the buggy side on the JSON path — divergence
...
fn trafilatura_json_gate() {
    ...
    for fixture_rel in FIXTURES {
        ...
        assert!(
            path.is_file(),
            "M7 Stage 2 fixture missing: {} (expected at {})",
            fixture_rel,
            path.display(),
        );
        ...
        // 1. Rust json output.
        let rust_json_raw = match extract_to_json(&html, None, &Options::default()) {
            Ok(s) => s,
            Err(e) => {
                report.push_str(&format!(
                    "  ERR   {} — extract_to_json returned Err: {e:?}\n",
                    fixture_rel,
                ));
                bucket_content += 1;
                continue;
        ...
        // 2. Python json output (subprocess oracle).
        let python_json_raw = match python_json(&path) {
            Ok(s) => s,
            Err(e) => panic!(
                "M7 STAGE 2 GATE: Python oracle failure on {} — {e}",
                fixture_rel,
            ),
        };

        // 3. NFC-normalise both (belt-and-braces).
        let rust_json: String = rust_json_raw.as_str().nfc().collect();
        let python_json: String = python_json_raw.as_str().nfc().collect();
        ...
        assert!(
            ...
            "M7 json gate: fixture {basename} appears in BOTH allowlist and deferred lists; \
             pick one — allowlist = anti-inversion-clean Python bug, deferred = mdrcel defect",
        );

        let bucket = classify(&rust_json, &python_json);
        if allowlisted {
        ...
        let first_diff_byte = first_diff_index(rust_json.as_bytes(), python_json.as_bytes());
        let rust_window = window_around(&rust_json, first_diff_byte, 100);
        let python_window = window_around(&python_json, first_diff_byte, 100);
        ...
        report.push_str(&format!(
            ...
            fixture_rel,
            tag,
            rust_json.chars().count(),
            python_json.chars().count(),
            first_diff_byte,
            escape(&rust_window),
    ...
    eprintln!("\n=== M7 json corpus gate verdict (BLOCKER) ===");
    eprintln!(
    ...
    assert_eq!(
        accounted, total,
        "M7 json gate accounting drift: pass={pass}, empty={bucket_empty}, \
    ...
    if pass + allowlist_python_bug + deferred_known_defect != total {
        panic!(
            "M7 json gate divergence: {pass}/{total} substantive + \
...
/// Python oracle path: spawn `run.py --json` and read its stdout as the JSON
/// payload. Bypasses the venv re-exec via `MDRCEL_TRAFILATURA_REEXECED=1`
/// (same trick as the txt / markdown / Stage 1b / 3-B gates).
fn python_json(snapshot_path: &Path) -> Result<String, String> {
